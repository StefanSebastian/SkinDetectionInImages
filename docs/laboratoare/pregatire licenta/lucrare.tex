...
\begin{document}
	...
	Texture, while easily identifiable by a human observer, does not have precise definition\cite{computer_vision_book}. Consequently, there are many choices for building a texture classification model depending on the author’s interpretation. A good example that illustrates the complexity of defining texture is the surface of wood, which contains intensity variations that despite being nonuniform can compose certain repeated patterns.
	
	Texture provides information about the spatial arrangement of intensities in an image\cite{computer_vision_book}. There are two main approaches in studying image texture: structural and statistical. The statistical method sees texture as a composition of primitives called texels arranged in a regular or repeated manner. Take a chess board for example - all squares are repeated in a predictable way. \todo{secon section}
