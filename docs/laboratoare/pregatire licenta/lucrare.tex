...
\begin{document}
	...
	The structural method sees texture as a composition of primitives called texels(microtexture) arranged in a regular or repeated manner(macrotexture). Take a chess board for example - all squares are repeated in a predictable way. These texels can be isolated using Voronoi polygons\cite{computer_vision_book}. This approach is more useful for generating textures than analyzing them because in nature the difference between micro and macro textures is not very clear and the their variability is high.
	
	In real life the second approach is much more common because finding and separating texels in natural textures is more difficult than computing some statistics on the gray levels of the image. These have also been shown to perform better than the other approaches, mainly through the use of second order statistics\cite{texture_analysis_review}.
	...
	\subsection{First-order histogram based features}
